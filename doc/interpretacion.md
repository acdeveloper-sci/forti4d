...
## Start Here: `reporte_consolidado.csv`

The consolidated report is the primary analysis surface. It has one row per
program unit and 25 columns covering size, complexity, coupling, density,
reachability, and COMMON block usage.
...
The following sequence identifies the highest-priority work items:
...
3. **Undocumented large units:** Filter `SLOC_neto > 100` and
   `Pct_Comentario < 5%`. These are maintenance liabilities — hard to
   understand without documentation.

4. **Legacy hotspots:** Filter `Pct_Legacy > 10%` and `Fan_In > 3`.
   COMMON/GOTO/EQUIVALENCE code that is also widely called is the
   riskiest to modify.

5. **Isolated algorithms:** Filter `Pct_Calculo > 50%` and `Fan_In < 3`
   and `Pct_Legacy < 5%`. These are the easiest units to extract, test,
   and migrate — start here if the goal is modernization.

`reporte_estrategia_migracion.csv` (from `analisis_cruzado.py`) applies a
similar rule-based approach automatically and provides a recommended action
for every unit.
