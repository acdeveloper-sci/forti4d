"""
test_clones.py
Verifies report_clones.csv produced by clones.py.
...
def test_identico_count(clones):
    identical = [r for r in clones if r["Status"].strip().upper() == "IDENTICAL"]
    assert len(identical) == 2


def test_similar_count(clones):
    similar = [r for r in clones if r["Status"].strip().upper() == "SIMILAR"]
    assert len(similar) == 3


def test_divergido_count(clones):
    diverged = [r for r in clones if r["Status"].strip().upper() == "DIVERGED"]
...
def test_math_util_a_is_identico(clones):
    ...
    assert len(rows) == 1
    assert float(rows[0]["Similarity_Pct"]) == 100.0
...
def test_math_util_b_is_similar(clones):
    ...
    assert len(rows) == 1
    assert float(rows[0]["Similarity_Pct"]) >= 75.0
...
def test_all_pairs_involve_utils_files(clones):
    ...
    for row in clones:
        files = {row["File_A"].strip(), row["File_B"].strip()}
        assert files == {"utils.f90", "utils_copy.f90"}
